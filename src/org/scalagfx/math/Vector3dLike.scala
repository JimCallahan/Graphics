...
//--------------------------------------------------------------------------------------------------
//   V E C T O R   3 D   L I K E                                                              
//--------------------------------------------------------------------------------------------------

/** A common set of methods for immutable 3-dimensional positions and vectors of Double
...
trait Vector3dLike 
{
  ...
  def toNative: DoubleBuffer = {
    val buf = ByteBuffer.allocateDirect(3 << 3).order(ByteOrder.nativeOrder).asDoubleBuffer
    putNative(buf)
    buf.rewind
    buf
  }
  ...
  def toNativeFloats: FloatBuffer = {
    val buf = ByteBuffer.allocateDirect(3 << 2).order(ByteOrder.nativeOrder).asFloatBuffer
    putNativeFloats(buf)
    buf.rewind
    buf
  }
